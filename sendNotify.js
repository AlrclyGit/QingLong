...
const push_config = {
  ...
  BARK_PUSH: 'https://bark.alrcly.com/d4irnTWLU9ScoQ3Laz5nyT', // bark IP 或设备码，例：https://api.day.app/DxHcxxxxxRxxxxxxcm/
  BARK_ARCHIVE: 1, // bark 推送是否存档
  BARK_GROUP: '青龙面板', // bark 推送分组
  ...
  PUSH_PLUS_TEMPLATE: 'html', // pushplus 发送模板，支持 html,txt,json,markdown,cloudMonitor,jenkins,route,pay
  PUSH_PLUS_CHANNEL: 'wechat', // pushplus 发送渠道，支持 wechat,webhook,cp,mail,sms
  PUSH_PLUS_WEBHOOK: '', // pushplus webhook 编码，可在 pushplus 公众号上扩展配置出更多渠道
  PUSH_PLUS_CALLBACKURL: '', // pushplus 发送结果回调地址，会把推送最终结果通知到这个地址上
  PUSH_PLUS_TO: '', // pushplus 好友令牌，微信公众号渠道填写好友令牌，企业微信渠道填写企业微信用户 id

  // 微加机器人，官方网站：https://www.weplusbot.com/
  WE_PLUS_BOT_TOKEN: '', // 微加机器人的用户令牌
  WE_PLUS_BOT_RECEIVER: '', // 微加机器人的消息接收人
  WE_PLUS_BOT_VERSION: 'pro', //微加机器人调用版本，pro 和 personal；为空默认使用 pro(专业版)，个人版填写：personal
  ...
  */
  ...
  CHRONOCAT_QQ: '', // 个人：user_id=个人 QQ 群则填入 group_id=QQ 群 多个用英文;隔开同时支持个人和群
  CHRONOCAT_TOKEN: '', // 填写在 CHRONOCAT 文件生成的访问密钥
  ...
  NTFY_URL: '', // ntfy 地址，如 https://ntfy.sh，默认为 https://ntfy.sh
  NTFY_TOPIC: '', // ntfy 的消息应用 topic
  NTFY_PRIORITY: '3', // 推送消息优先级，默认为 3
  NTFY_TOKEN: '', // 推送 token，可选
  ...
  WXPUSHER_TOPIC_IDS: '', // wxpusher 的 主题 ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
  WXPUSHER_UIDS: '', // wxpusher 的 用户 ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
};
  ...
  return new Promise((resolve) => {
    ...
    if (GOTIFY_URL && GOTIFY_TOKEN) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('Gotify 发送通知调用 API 失败😞\n', err);
          } else {
            if (data.id) {
              console.log('Gotify 发送通知消息成功🎉\n');
            } else {
              console.log(`Gotify 发送通知调用 API 失败😞 ${data.message}\n`);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
  ...
  return new Promise((resolve) => {
    ...
    if (GOBOT_URL) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('Go-cqhttp 通知调用 API 失败😞\n', err);
          } else {
            if (data.retcode === 0) {
  ...
  return new Promise((resolve) => {
    ...
    if (PUSH_KEY) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('Server 酱发送通知调用 API 失败😞\n', err);
          } else {
  ...
  return new Promise((resolve) => {
    ...
    if (DEER_KEY) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('PushDeer 通知调用 API 失败😞\n', err);
          } else {
            // 通过返回的 result 的长度来判断是否成功
  ...
  return new Promise((resolve) => {
    ...
    if (CHAT_URL && CHAT_TOKEN) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('Chat 发送通知调用 API 失败😞\n', err);
          } else {
            if (data.success) {
        ...
        try {
          if (err) {
            ...
            if (data.ok) {
              ...
              console.log('请主动给 bot 发送一条消息并检查接收用户 ID 是否正确。\n');
    ...
    if (QYWX_AM) {
      ...
      $.post(options_accesstoken, (err, resp, json) => {
        ...
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              console.log('成员 ID:' + ChangeUserId(desp) + '企业微信应用消息发送通知消息失败😞\n', err);
            } else {
              if (data.errcode === 0) {
                console.log('成员 ID:' + ChangeUserId(desp) + '企业微信应用消息发送通知消息成功🎉。\n');
              } else {
  ...
  return new Promise((resolve) => {
    ...
    if (PUSH_PLUS_TOKEN) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log(`pushplus 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息失败😞\n`, err);
          } else {
            if (data.code === 200) {
              console.log(
                `pushplus 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知请求成功🎉，可根据流水号查询推送结果：${
                  data.data
                }\n注意：请求成功并不代表推送成功，如未收到消息，请到 pushplus 官网使用流水号查询推送最终结果`
              );
            } else {
              console.log(`pushplus 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息异常 ${data.msg}\n`);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
  ...
  return new Promise((resolve) => {
    ...
    if (PUSHME_KEY) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('PushMe 发送通知调用 API 失败😞\n', err);
          } else {
            if (data === 'success') {
    ...
    for (const [chat_type, ids] of [
      ...
      for (const chat_id of ids) {
        ...
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              console.log('Chronocat 发送 QQ 通知消息失败😞\n', err);
            } else {
              if (chat_type === 1) {
...
function webhookNotify(text, desp) {
  return new Promise((resolve) => {
    ...
    if (!WEBHOOK_METHOD || !WEBHOOK_URL || (!WEBHOOK_URL.includes('$title') && !WEBHOOK_BODY.includes('$title'))) {
      resolve();
      return;
    }
    ...
    const body = parseBody(WEBHOOK_BODY, WEBHOOK_CONTENT_TYPE, (v) =>
      v?.replaceAll('$title', text?.replaceAll('\n', '\\n'))?.replaceAll('$content', desp?.replaceAll('\n', '\\n'))
    );
  ...
  return new Promise((resolve) => {
    ...
    if (WXPUSHER_APP_TOKEN) {
      ...
      $.post(options, (err, resp, data) => {
        try {
          if (err) {
            console.log('wxpusher 发送通知消息失败！\n', err);
          } else {
            if (data.code === 1000) {
              console.log('wxpusher 发送通知消息完成！');
            } else {
              console.log(`wxpusher 发送通知消息异常：${data.msg}`);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
